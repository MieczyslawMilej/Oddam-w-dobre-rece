const data = {
  fundations: {
    bio:
      "W naszej bazie znajdziesz listę zweryfikowanych Fundacji, z którymi współpracujemy. Możesz sprawdzić czym się zajmują, komu pomagają i czego potrzebują.",
    description: [
      {
        name: "Dbam o Zdrowie",
        mission: "Pomoc osobom znajdującym się w trudnej sytuacji życiowej.",
        stuff_needed: ["ubrania", "jedzenie", "sprzęt AGD", "meble", "zabawki"]
      },
      {
        name: "Dla dzieci",
        mission: "Pomoc dzieciom z ubogich rodzin.",
        stuff_needed: ["ubrania", "meble", "zabawki"]
      },
      {
        name: "Bez domu",
        mission: "Pomoc dla osób nie posiadających miejsca zamieszkania.",
        stuff_needed: ["ubrania", "jedzenie", "ciepłe koce"]
      },
      {
        name: "Dbam o Zdrowie 1",
        mission: "Pomoc osobom znajdującym się w trudnej sytuacji życiowej.",
        stuff_needed: ["ubrania", "jedzenie", "sprzęt AGD", "meble", "zabawki"]
      },
      {
        name: "Dla dzieci 1",
        mission: "Pomoc dzieciom z ubogich rodzin.",
        stuff_needed: ["ubrania", "meble", "zabawki"]
      },
      {
        name: "Bez domu 1",
        mission: "Pomoc dla osób nie posiadających miejsca zamieszkania.",
        stuff_needed: ["ubrania", "jedzenie", "ciepłe koce"]
      },
      {
        name: "Dbam o Zdrowie 2",
        mission: "Pomoc osobom znajdującym się w trudnej sytuacji życiowej.",
        stuff_needed: ["ubrania", "jedzenie", "sprzęt AGD", "meble", "zabawki"]
      },
      {
        name: "Dla dzieci 2",
        mission: "Pomoc dzieciom z ubogich rodzin.",
        stuff_needed: ["ubrania", "meble", "zabawki"]
      },
      {
        name: "Bez domu 2",
        mission: "Pomoc dla osób nie posiadających miejsca zamieszkania.",
        stuff_needed: ["ubrania", "jedzenie", "ciepłe koce"]
      }
    ]
  },

  organizations: {
    bio:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation.",
    description: [
      {
        name: "Lorem Ipsum 1",
        mission:
          "Quis varius quam quisque id diam vel quam elementum pulvinar.",
        stuff_needed: ["Egestas", "sed", "tempus"]
      },
      {
        name: "Lorem Ipsum 2",
        mission:
          "Hendrerit gravida rutrum quisque non tellus orci ac auctor augue.",
        stuff_needed: ["Ut", "aliquam", "purus", "sit", "amet"]
      },
      {
        name: "Lorem Ipsum 3",
        mission: "Scelerisque in dictum non consectetur a erat nam.",
        stuff_needed: ["Mi", "quis", "hendrerit", "dolor"]
      },
      {
        name: "Lorem Ipsum 4",
        mission:
          "Quis varius quam quisque id diam vel quam elementum pulvinar.",
        stuff_needed: ["Egestas", "sed", "tempus"]
      },
      {
        name: "Lorem Ipsum 5",
        mission:
          "Hendrerit gravida rutrum quisque non tellus orci ac auctor augue.",
        stuff_needed: ["Ut", "aliquam", "purus", "sit", "amet"]
      },
      {
        name: "Lorem Ipsum 6",
        mission: "Scelerisque in dictum non consectetur a erat nam.",
        stuff_needed: ["Mi", "quis", "hendrerit", "dolor"]
      }
    ]
  },
  collections: {
    bio:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation.",
    description: [
      {
        name: "Collection Ipsum 1",
        mission:
          "Quis varius quam quisque id diam vel quam elementum pulvinar.",
        stuff_needed: ["Egestas", "sed", "tempus"]
      },
      {
        name: "Collection Ipsum 2",
        mission:
          "Hendrerit gravida rutrum quisque non tellus orci ac auctor augue.",
        stuff_needed: ["Ut", "aliquam", "purus", "sit", "amet"]
      },
      {
        name: "Collection Ipsum 3",
        mission: "Scelerisque in dictum non consectetur a erat nam.",
        stuff_needed: ["Mi", "quis", "hendrerit", "dolor"]
      }
    ]
  }
};

export default data;
